...
<div id="comments" class="comments-area">
	...
	if ( have_comments() ) : ?>
		<h2 class="comments-title">
			<?php
					esc_html_e( 'Comments', 'spa-and-salon' );
			?>
		</h2>
    ...
    //https://codex.wordpress.org/Function_Reference/comment_form
        ...
        $args = array(
          ...
          'must_log_in' => '<p class="must-log-in">' .
            sprintf(
              esc_html__( 'You must be <a href="%s">logged in</a> to post a comment.', 'spa-and-salon' ),
              wp_login_url( apply_filters( 'the_permalink', get_permalink() ) )
            ) . '</p>',
        
          'logged_in_as' => '<p class="logged-in-as">' .
            sprintf(
            esc_html__( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'spa-and-salon' ),
          ...
          'comment_notes_before' => '<p class="comment-notes"><span class="email-notes">' .
            esc_html__( 'Your email address will not be published. ', 'spa-and-salon' ) . '</span>' . ( $req ? $required_text : '' ) .
