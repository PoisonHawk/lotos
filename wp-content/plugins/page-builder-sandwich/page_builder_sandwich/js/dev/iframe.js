...
var ready = function() {
	...
	var url = window.location.toString().replace( /[&\?]?pbs_iframe(=\d)?/g, '' );
...
window.addEventListener( 'message', function receiveMessage( ev ) {
	var iframe = document.querySelector( 'iframe' );
	if ( ev.data === 'pbs_iframe_change' && window.location.href !== iframe.contentWindow.location.href && window.location.href.indexOf( iframe.contentWindow.location.href ) === -1 ) {
	...
	} else if ( ev.data === 'pbs_iframe_change' && window.location.href !== iframe.contentWindow.location.href && window.location.href.indexOf( iframe.contentWindow.location.href ) !== -1 ) {
...
window.pbsGoResponsive = function( type ) {
	...
	setTimeout( function() {
		...
		Array.prototype.forEach.call( emptyElements, function( el ) {
			el.parentNode.style.display = view === 'phone' || view === 'tablet' ? 'none' : '';
		} );
