		...
		wp.hooks.addAction( 'pbs.element.out', function( element ) {
			if ( this.element && element && element._domElement ) {
				if ( this.element._domElement === element._domElement || element._domElement.contains( this.element._domElement ) )  {
					if ( this.constructor.name.toLowerCase().indexOf( 'toolbar' ) === 0 ) {
						this._hide();
					}
				}
			}
...
( function() {
	...
	var pbsOverElement = _.throttle( function( target ) {
		...
		if ( currentOverElement !== null && target._ceElement ) {
...
PBSEditor.OverlayControls = ( function( _super ) {
	...
	OverlayControls.prototype.updatePosition = function( element ) {
		...
		var styles = element._cachedStyles;
		var rect = element._cachedRect;
		var toolbar = element._cachedToolbar;
		var scrollY = element._cachedScrollY;
		var toolbarHeight = element._cachedToolbarHeight;
		if ( ! toolbar || +new Date() - lastUpdateTime > 30 ) {
			styles = getComputedStyle( element._domElement );
			rect = element._domElement.getBoundingClientRect();
			toolbar = getComputedStyle( document.querySelector( '.pbs-toolbox-bar' ) );
			scrollY = window.scrollY || window.pageYOffset;
			toolbarHeight = parseInt( toolbar.height, 10 );
			element._cachedStyles = styles;
			element._cachedRect = rect;
			element._cachedToolbar = toolbar;
			element._cachedScrollY = scrollY;
			element._cachedToolbarHeight = toolbarHeight;
		...
		var controls = [
			{
				...
				onMoveStart: function( overlay, element, styles, rect ) {
					...
					var label = rect.width + ' &times; ' + parseInt( rect.height, 10 ) + ' px';
					if ( element.constructor.name === 'Image' ) {
				...
				onMove: function( overlay, element, deltaX ) {
					...
					if ( element.constructor.name === 'Image' && width > rect.width ) {
...
PBSEditor.OverlayRow = (function(_super) {
	...
	OverlayRow.prototype.show = function( element ) {
		...
		// this._bottomResize.setAttribute( 'data-label', pbsParams.labels.margin + ': ' + styles.marginBottom );
		// this._bottomResize.style.height = styles.marginBottom;
		// this._bottomResize.setAttribute( 'data-value', 'margin-' + styles.marginBottom );
		...
		// If row is nested, add a class.
		if ( element.parent().constructor.name === 'DivCol' ) {
	...
	OverlayRow.prototype.onMoveStart = function() {
		...
		if ( parseInt( rect.height, 10 ) > this.resizeBottomInitialValue ) {
			this.resizeBottomInitialValue = parseInt( rect.height, 10 );
		}
	...
	OverlayRow.prototype.onMove = function() {
		var margin, remainder, height, rect, totalWidth, totalWidthNoMargin, i;
		...
		} else if ( this._locationDragging === 'resize-bottom' ) {
	...
	OverlayRow.prototype.onClick = function( ev ) {
		...
		// Resize indicator.
		if ( this._locationDragging === 'resize-bottom' ) {
			var styles = getComputedStyle( this.element._domElement );
			var rect = this.element._domElement.getBoundingClientRect();
			this._overlaySize = document.createElement( 'DIV' );
			this._overlaySize.classList.add( 'pbs-size-indicator' );

			var label = parseInt( styles.minHeight, 10 );
			if ( parseInt( rect.height, 10 ) > label ) {
				label = parseInt( rect.height, 10 );
			}
			...
			var toolbarHeight = parseInt( toolbar.height, 10 );
			var scrollY = window.scrollY || window.pageYOffset;
			this._overlaySize.style.top = ( rect.top + scrollY - toolbarHeight ) + 'px';
			this._overlaySize.style.left = rect.right + 'px';
			document.body.appendChild( this._overlaySize );
		}
	};
...
PBSEditor.Toolbar = ( function( _super ) {
	...
	Toolbar.prototype.checkIfTooSmall = function() {
				...
				},
				{
					...
					display: function( element, button ) {
						...
						if ( window.innerWidth > 400 ) {
							...
							if ( element.attr( 'data-pbs-hide-phone' ) === '1' || element.attr( 'data-pbs-hide-phone' ) === '2' ) {
								button.classList.add( 'pbs-responsive-hidden' );
							} else {
								button.classList.remove( 'pbs-responsive-hidden' );
							}
					...
					onClick: function( element, button ) {
						if ( window.innerWidth > 400 ) {
							...
							if ( element.attr( 'data-pbs-hide-tablet' ) ) {
								...
								if ( element.attr( 'data-pbs-hide-phone' ) === '2' ) {
									element.removeAttr( 'data-pbs-hide-phone' );
								}
								button.classList.remove( 'pbs-responsive-hidden' );
							} else {
								element.attr( 'data-pbs-hide-tablet', '1' );
								if ( element.attr( 'data-pbs-hide-phone' ) !== '0' && element.attr( 'data-pbs-hide-phone' ) !== '1' ) {
							...
							if ( element.attr( 'data-pbs-hide-phone' ) !== '0' ) {
...
window.PBSEditor.premiumTools = {
	...
	'font-up': pbsParams.labels.increase_font_size + ' (' + pbsParams.labels.premium + ')',
